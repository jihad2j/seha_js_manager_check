...
mongoose.connect('mongodb://localhost:27017/masar', {
  useNewUrlParser: true,
  useUnifiedTopology: true
}).then(function () {
  console.log('Connected to the database'); // Perform database operations here
})["catch"](function (err) {
  console.error('Error connecting to the database', err);
});

var Customer = require('../models/customerSchema.cjs'); // Get data from Mongoose DB
...
Customer.find({}).then(function (result) {
  ...
  var imagesBack = path.join(__dirname, '..', 'public', 'image', 'back1.jpg');
  console.log(imagesBack);
  doc.image(imagesBack, 0, 0, {
    width: 595.28,
    height: 841.89
  });
  var logo_seha = path.join(__dirname, '..', 'public', 'image', 'logo-seha.jpg');
  doc.image(logo_seha, 26, 20, {
    width: 90,
    height: 50
  });
  var imageslogo_kingdom = path.join(__dirname, '..', 'public', 'image', 'logo-kingdom.jpg');
  doc.image(imageslogo_kingdom, 235, 20, {
    width: 125,
    height: 40
  });
  var imageslogo_right = path.join(__dirname, '..', 'public', 'image', 'logo-right.jpg');
  doc.image(imageslogo_right, 430, 1.7, {
    width: 181,
    height: 140
  });
  var imagesbarcode_new = path.join(__dirname, '..', 'public', 'image', 'barcode-new.jpg');
  var imageslogo_health = path.join(__dirname, '..', 'public', 'image', 'logo-health.jpg');
  ...
  doc.font(fontPathcSemiBold);
  doc.fillColor('#0f0e0e');
  var imagelogo = path.join(__dirname, '..', 'public', 'image', logocenter + '.jpg');
  ...
  doc.fontSize(7);
  var line = path.join(__dirname, '..', 'public', 'image', 'line.jpg');
